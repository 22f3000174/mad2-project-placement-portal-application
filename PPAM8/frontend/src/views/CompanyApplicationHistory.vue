    ...
    async exportHistory(format) {
      try {
        ...
        const response = await axios.post(
          "http://127.0.0.1:5000/company/application-history/export",
          {
        ...
        const interval = setInterval(async () => {
          try {
            const status = await axios.get(
              `http://127.0.0.1:5000/company/application-history/export-status/${taskId}`
            )
            ...
            if (status.data.state === "FAILURE") {
              clearInterval(interval)
              alert(
                status.data.error || "Export failed"
              )
            }
            ...
            alert(
              ...
              "Failed while checking export status"
            )
          }
        }, 2000)
